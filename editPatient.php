<?php
    ...
    if(isset($_GET["id"])){
        $id = $_GET["id"];
        $getPatient = "SELECT * FROM patients WHERE id=$id";
        $result = $conn->query($getPatient);
        ...
        $associated_doctor = $patient["associated_doctor"];
        ...
        if( !empty($firstname)
            ...
            if(filter_var($_POST["email"], FILTER_VALIDATE_EMAIL)){
                ...
                if ($conn->query($update_query) === TRUE) {
                    $ALERT["message"] = "patient modifier";
                    $ALERT["classValue"] = "alert-success";
                } else {
                    $ALERT["message"] = "patient est pas modifier";
        ...
        <form class="w-auto shadow p-3 m-3 border rounded" method="POST" action="<?php echo $_SERVER["PHP_SELF"]; ?>" autocomplete="true">
            ...
            <div class="row">
                ...
                <div class="col">
                    ...
                    <button type="submit" name="editPatient" class="btn btn-success w-100">ENREGISTRER</button>
                </div>
